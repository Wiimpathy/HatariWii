...
#ifdef GEKKO
 	{ SGBUTTON, SG_DEFAULT, 0, 16,22, 20,1, "Back to settings" },
	{ SGBUTTON, SG_CANCEL, 0, 3,800, 2,1, "B" },	// hidden, back with B button
#else
...
void Dialog_DeviceDlg(void)
{
	...
	while (but != DEVDLG_EXIT && but != SDLGUI_QUIT
	       && but != SDLGUI_ERROR && !bQuitProgram && but != DEVDLG_BACK);
