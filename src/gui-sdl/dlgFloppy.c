...
#ifdef GEKKO
#define FLOPPYDLG_BACK        25
extern void DlgDisk_BrowseDisk(char *dlgname, int drive, int diskid);
extern char lastgame[256];
#endif
...
#ifdef GEKKO
	{ SGBUTTON, SG_DEFAULT, 0, 22,18, 20,1, "Back to settings" },
...
#ifdef GEKKO
	/* Save the name to know what to do for the snapshots */
	sprintf (lastgame, selname);
#endif
...
#ifdef GEKKO
	while (but != FLOPPYDLG_EXIT && but != FLOPPYDLG_BACK && but != SDLGUI_QUIT 
